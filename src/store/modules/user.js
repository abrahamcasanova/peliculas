...
const actions = {
  ...
  login({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      login({ email: username.trim(), password: password, remember_me: false }).then(response => {
        const data = response
  ...
  getInfo({ commit, state }) {
    return new Promise((resolve, reject) => {
      getInfo(state.token, state.id).then(response => {
        const data = response
        ...
        const { routes, roles, user, avatar, introduction } = data
